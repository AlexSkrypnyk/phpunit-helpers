...
final class BenchmarkComparisonTest extends TestCase {
  ...
  public function testReportsHeadWhenBaseCarriesNoBenchmarks(): void {
    $base = $this->workspace . '/base';
    $this->assertTrue(mkdir($base, 0777, TRUE));
    $head = $this->createCheckout('head', 1000);

    [$exit_code, $output] = $this->compare(['--base=' . $base, '--head=' . $head]);

    $this->assertSame(0, $exit_code, 'A base without benchmarks must be measured without a comparison. Output: ' . $output);
